	...
	public partial class proview : UIViewController
	{
		...
		void Internal_ViewDidLoad(bool v)
		{
			try
			{
				...
				StoreDataModel.Items.Add(LoggingClass.txtstore1);
				StoreDataModel.Items.Add(LoggingClass.txtstore2);
				StoreDataModel.Items.Add(LoggingClass.txtstore3);
				...
				txtFirst3= new UITextField
				{
					...
					Font = UIFont.FromName("HelveticaNeue", 13f),
					//DataDetectorType = UIDataDetectorType.PhoneNumber
					//UserInteractionEnabled = true

				};
				txtFirst3.EditingChanged += delegate {
					Console.WriteLine("Entered");
					//string phn3=txtFirst3.Text.Length;
					if (txtFirst3.Text.Length == 3)
					{
						Console.WriteLine(txtFirst3.Text.Length);
						txtSecond3.BecomeFirstResponder();

						//Console.WriteLine(txtSecond3.CanBecomeFirstResponder.ToString());
					}
				};
				...
				txtSecond3.EditingChanged += delegate {
					//Console.WriteLine("Entered");
					//string phn3=txtFirst3.Text.Length;
					if (txtSecond3.Text.Length == 3)
					{
						//Console.WriteLine(txtFirst3.Text.Length);
						txtLast4.BecomeFirstResponder();
						//Console.WriteLine(txtSecond3.CanBecomeFirstResponder.ToString());
					}
					if (txtSecond3.Text.Length == 0)
					{
						txtFirst3.BecomeFirstResponder();
					}
				...
				txtLast4.EditingChanged += delegate {
					//Console.WriteLine("Entered");
					//string phn3=txtFirst3.Text.Length;
					if (txtLast4.Text.Length == 4)
					{
						//Console.WriteLine(txtFirst3.Text.Length);
						txtLast4.ResignFirstResponder();
						//Console.WriteLine(txtSecond3.CanBecomeFirstResponder.ToString());
					}
					if (txtLast4.Text.Length == 0)
					{
						txtSecond3.BecomeFirstResponder();
					}
				...
				stpicker.Frame = new CGRect(strtx, y, 50, 70);
				...
				txtZipcode = new UITextField
				{
					...
					Frame = new CGRect(zipx+70, y+25, 60, ScreenWidth),
					BorderStyle=UITextBorderStyle.RoundedRect,
					UserInteractionEnabled = true,
					KeyboardType=UIKeyboardType.NumberPad,
					Font = UIFont.FromName("HelveticaNeue", 13f)
				...
				{
					Text="Expiry:",
					Frame=new CGRect(zipx,y,ScreenHeight,ScreenWidth),
					Font = UIFont.FromName("HelveticaNeue-Bold", 16f)
				};

				UILabel lblCardDate = new UILabel
				{
					Frame = new CGRect(zipx+70, y, 80, ScreenWidth),
					Font = UIFont.FromName("HelveticaNeue", 13f)
				};
				//y = y + Space;
				//y = y - 10;
				...
				StorePicker.Frame = new CGRect(strtx, y, UIScreen.MainScreen.Bounds.Width-(strtx+10), 70);
				//StorePicker.Set
				...
				else
				{
					...
					if (cRes.customer.CardNumber != null && cRes.customer.CardNumber != "")
					{
						...
						btnUpdate.TouchUpInside += async delegate
						{
							...
							else if (txtFirst3.Text.Length == 0 )
							{
								BTProgressHUD.ShowErrorWithStatus("Please enter phone number", 3000);
							}
							...
							else
							{
								...
								{
									cust.PreferredStore = Convert.ToInt32(CurrentUser.GetStore());
									StorePicker.Select(cust.PreferredStore, 0, false);
